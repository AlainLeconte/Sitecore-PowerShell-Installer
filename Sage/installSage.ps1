...
$configPath = "C:\Sage\SitecoreInstallation\install.Imagine.Sage.Dev.CM.config"
...
function Test-SqlInstallPath
{
    ...
    $dbInstallPath = Get-DatabaseInstallFolderPath $TRUE
    $dbUncInstallPath = Get-DatabaseInstallFolderPath $FALSE
    Write-Message "Test-SqlInstallPath / dbInstallPath : $dbInstallPath" "White" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
    Write-Message "Test-SqlInstallPath / dbUncInstallPath : $dbUncInstallPath" "White" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
    #if (-Not(Test-Path $dbInstallPath))
    #{
    #    Write-Message "Path does not exist: $dbInstallPath" "Red" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
    #    return $FALSE
    #}
    if (!(Test-Path $dbUncInstallPath))
    {
        Write-Message "UNC Path does not exist: $dbUncInstallPath" "Red" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
        return $FALSE
    }


    $isCorrectRights = $FALSE
    # Check that SQL has correct rights over install path, else Database Attach will fail
    [Microsoft.SqlServer.Management.Smo.Server]$sqlServerSmo = Get-SqlServerSmo
    $user = $sqlServerSmo.SqlDomainGroup
    Write-Message "SQL User : $user" "White" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
    # $acl = Get-Acl $dbUncInstallPath
    $sqlServerName = $script:configSettings.Database.SqlServerName
    $scriptBlock = [ScriptBlock]::Create('$acl = get-acl "'+$dbInstallPath+'"; return $acl')
    $acl = Invoke-Command -Computer $sqlServerName -ScriptBlock $sb  
    Write-Message ($acl.AccessToString) -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
    foreach ($ac in $acl) {
        Write-Message "zzzz" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
        $user= $ac.Access.IdentityReference
        Write-Message $user -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
    }
    $isCorrectRights = $acl.Access.IdentityReference | Where {($_.IdentityReference -like $user) -and ($_.FileSystemRights -like "FullControl")}
    Write-Message $isCorrectRights -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
...
function Test-SupportedSitecoreVersion
{
    ...
    Write-Message "Sitecore version is : $versionToInstall" -WriteToLog $FALSE -HostConsoleAvailable $hostScreenAvailable
...
function Set-ConfigurationFiles
{
    ...
    if ($script:configSettings.WebServer.CDServerSettings.DisableUploadWatcher)
    {
        ...
        if($node){
            Write-Message $node "Gray" -WriteToLog $TRUE -HostConsoleAvailable $hostScreenAvailable
